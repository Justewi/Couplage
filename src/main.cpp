...
int main(int argc, char** argv) {
    ...
    if (argc < 4 || argc > 5) {
        std::cerr << "Le nombre de paramètre est incorrect" << std::endl
                << "Utilisation : " << argv[0] << " Type_de_graph Nombre_de_noeud pas (optionnal:nb_repetition)" << std::endl;
        return 0;
    }
    ...
    for (std::pair<float, int> i : results) {
        std::cout <<"Proba lien : "<< i.first << ", nombre de parfaits :" << i.second << ", proba : " << ((float) i.second / repetition) << std::endl;
    }
